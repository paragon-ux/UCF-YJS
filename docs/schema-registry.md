...
| `compatibility` | An explicit named policy such as `identity`, `m0_outcome_chain_frontier`, or `reserved_for_m1` |
| `migrations.kind` | `identity` in M1.2 |

`supported` entries are readable and writable by the current M1.2 code.
`reserved` entries are named for M1 compatibility planning but are not read or
written until their feature implements the storage or log surface.

## Registered Artifacts

| Artifact | Current version | M1.2 write status |
| --- | --- | --- |
...
| Workspace generation schema | `ucf-yjs.workspace_generation.v1` | Reserved, unsupported |
| Citation domain schema | `ucf-yjs.citations.v1` | Supported |
| Reducer version | `ucf-yjs.reducer.v1` | Supported |

## Migration Policy

M1.2 implemented identity migrations only. M1.3 adds the forward-only
...
`agent_view.get` do not advance semantic identity.
